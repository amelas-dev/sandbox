...
export function RecordPreviewSelect() {
    ...
    if (!dataset) {
        return (_jsxs("div", { className: "flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-500 shadow-sm dark:border-slate-800 dark:bg-slate-900", children: [_jsx(ChevronDown, { className: "h-4 w-4 opacity-0" }), _jsx("span", { children: "No dataset loaded" })] }));
    }
    return (_jsxs("div", { className: "relative inline-flex items-center", children: [_jsx("select", { className: cn('appearance-none rounded-xl border border-slate-200 bg-white px-3 py-2 pr-8 text-sm font-medium shadow-sm focus:outline-none focus:ring-2 focus:ring-brand-500 dark:border-slate-800 dark:bg-slate-900'), value: previewIndex, onChange: (event) => setPreviewIndex(Number(event.target.value)), children: dataset.rows.map((_row, index) => {
