...
/**
 * Render an HTML artifact into a PDF blob using html-to-image + jsPDF.
 */
...
/**
 * Render a DOCX document with a plain-text fallback of the HTML artifact.
 */
...
/**
 * Wrap the artifact HTML in a blob for downloading as a static page.
 */
...
export async function exportArtifacts(
  ...
  if (!artifacts.length) {
    throw new Error('No artifacts were generated to export.');
  }
