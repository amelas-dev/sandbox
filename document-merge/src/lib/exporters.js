...
async function renderPdf(artifact) {
...
async function renderDocx(artifact) {
...
async function renderHtml(artifact) {
    return new Blob([artifact.html], { type: 'text/html' });
}
export async function exportArtifacts(artifacts, format) {
    const files = [];
